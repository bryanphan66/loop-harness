...
it closes) — latest **v6.20** (the backend/security stratum — a SECOND retrospective ran the mine→cluster→design→adversarial-verify pass over hasi-hub (a payments + multi-tenant + auth + cron + multi-pod backend, 166 reports), mined ~149 defect instances / 23 classes, deduped vs v6.19, and folded 14 NET-NEW gates — the security · concurrency · resilience · prod-packaging layer the UI-heavy elearning run never modelled (6 classes hasi shared were already caught by v6.19, validating generalization): thirteen phase-acceptance legs 15–27 (domain state-field integrity, object-level authz/IDOR, per-route rate-limit classification, session-lifecycle & OTP-purpose, canonical validator/identity-key, concurrency & write-path atomicity, process/request resilience, multi-instance state safety, FE↔BE contract fidelity, seed reseed-idempotency, notification/fan-out integrity, test-integrity, prod-image packaging + runtime-capability) — all backend, no new visual-fidelity block — plus a build-execution Phase-Preflight rule (resolve every cited symbol/path/import/id-type at HEAD before writing a phase) and self-QA group H; prior **v6.19** (retrospective enrichment — a multi-agent pass mined 182 human-caught defects from 83 elearning build/fix/QA reports, clustered to 23 classes, and folded the 13 recurring machine-checkable gaps the prior versions leaked: six phase-acceptance legs (9 route-reachability/no-orphan-route, 10 seed-coherence + prototype-count/copy-fidelity, 11 build+migration hygiene, 12 create/edit DTO round-trip, 13 record-lifecycle, 14 i18n-catalog+export) + a Leg-5 U-img public-image-integrity clause; seven visual-fidelity auto-blocks U13 dead-affordance · U14 inline-grid-reflow · U15 icon-registry-coverage · U16 enum-status-exhaustive · U17 prototype-copy-verbatim · U18 shared-primitive-clobber · U19 toast-convention; six new `lint:gates` scripts; and the keystone `playbooks/pre-demo-self-qa-checklist.md` — a runnable 7-group checklist the agent drives against the preview BEFORE any human handoff, turning screenshot-by-screenshot human review into agent self-check so the next run catches the long tail of "small" bugs at build time not at the demo; prior **v6.18** (user-facing product media is REAL, not re-drawn — two linked delivery-media lessons: a user guide's imagery must be **real captured screenshots inside the device frame**, not hand-drawn schematic SVG mockups that "look like the app" but can't be mapped to it (keep the frame, swap the interior; fall back to schematic only where a shot is missing) — the user-manual is held to the same fidelity as the UI; and a customer demo **VIDEO** is a **narrated product-tour**, not a step-by-step recording — `playbooks/demo-video-production.md` names three styles (user-guide Playwright per-step recording · Remotion product-tour with device-frame + voiceover + big→small storyline · live demo-script cheat-sheet) and says pick the style FIRST because the tool looks the same but the script differs; verified product-tour recipe = isolated Remotion project outside the app workspace, reuse the guide's real screenshots, offline neural TTS voiceover (piper vi_VN — no cloud key, deterministic), timeline driven by measured VO length, render per-part MP4 + stitched master, binaries gitignored; prior **v6.17** (Go-live hardening — the "Go-live" half of Macro 2 gains two floors + two DoD legs from elearning's hardcode sweep + prod deploys: **config-driven business identity** (`playbooks/config-driven-identity.md`) — brand/company legal entity/tax code/contact/canonical URL/SEO provider/copyright render from `site_configs`, never a code literal, because a hardcoded brand passes the demo glance (seeded value == literal) then breaks when the client edits Settings — cert PDF from the admin template not a baked layout, invoice seller + email footer + json-ld provider + copyright all config-driven; go-live grep sweep + flip-a-Settings-value proof; marketing social-proof numbers wired-to-real or kept by owner decision, never silently downgraded — and **go-live deploy verify-at-source** (`playbooks/go-live-deploy-verify.md`): build-time-inlined env (`NEXT_PUBLIC_*`) is a Docker build ARG not a runtime env and a same-commit redeploy reuses the image cache (the `localhost:3000`-in-prod symptom); verify by health `.status==ok` + a content marker only the new build produces, never CI-green/HTTP-200/a `commitSha` that reads `local-dev`; a compose `${MONEY_SECRET:-placeholder}` default is worse than missing (passes presence-only fail-closed → prod boots on a fake account) so money/identity guards reject known placeholders; a fail-closed hardening ships to real prod with real creds set first, never to the demo box it would crash-loop; the prod deploy is a named-endpoint human decision; plus five runtime+fidelity legs consolidated from the P0–P16 build — **boot-smoke the real AppModule** in validate (compile+unit-tests miss DI/fail-closed boot errors that crash-loop prod; optional-collaborator ctor params carry `@Optional()`), **media delivery is an HTTP proxy route not a raw signed-URL** (a `file://` local URL / relative child playlist spins hls.js forever — verify by driving the student path to `#EXTM3U` + a `.ts` 200), **interaction-completeness U11** (a screen that renders but whose deep-link/tab doesn't switch content — URL-derived state + shared toast), **screen-region completeness U12** (hero ported but StatCards/pagination/tabs/secondary regions dropped though data exists), and **fetch-all grids cap fetch ≤ the endpoint pageSize** (200-vs-max-100 → 422); prior **v6.16** (grid completeness floor: every data grid — a table of records — ships pagination + filter + sort as DoD (plus export where the REQ calls for it), elevated from per-feature to a universal crud-phase gate (Leg-8) so no record list ships as a bare unfilterable/unsortable table; a 2-D config matrix is exempt); prior **v6.15**: a webhook is authenticated to the PROVIDER's ACTUAL documented scheme — an HMAC body signature, an `Authorization: Apikey` header (SePay), basic-auth, mTLS — confirmed against their docs AND a real sandbox callback, not a guessed HMAC default that 401s every real callback and silently breaks the integration while a mocked test passes; prior **v6.14**: a new fail-closed prod-required secret must ship WITH its deploy-env value in the same phase — a safe sandbox default in the compose (`${VAR:-default}`) or a report line for the control to inject — or the next deploy crash-loops the API (health red, box 404) while the check itself was correct; prior **v6.13**: a ported screen must be the WHOLE design, not a reduced gist: every filter/toolbar control present + wired (a dropped select is a lost feature), shared chrome (utility top-strip, multi-column footer) ported at full fidelity not a stub, and a kit component rendered in the SAME variant the export picked for that screen (light nav where the export is light, not the dark on-ink variant) — size-S is not licence for a lossy port (U10); prior **v6.12**: a typed entity — a lesson `video|text|pdf`, any `type`/`kind` enum with SRS-distinct content — must render PER TYPE: the editor branches on the enum and surfaces each value's real content (video asset / rich-text body / PDF), never the single variant the prototype happened to draw defaulted onto all (U9); prior **v6.11**: a working native browser primitive is NOT the adopted design: a native `<video controls>` grey bar where the export shows a designed player (poster + center play + res/duration badge), or a `window.prompt` where it shows an in-app dialog, is redraw-by-omission (U8) — adopt the designed control, native only where the export shows it; and a transcode ladder never upscales — probe source height, emit only rungs ≤ source (≤480p → one 480p rung, no fake 720/1080), badge/DB carry the real max height; prior **v6.10**: authz isn't finished at the API: the Security floor grows two legs — every admin screen gates its mutating controls (delete/publish/create) on the caller's fresh grant surfaced via `/auth/me`, hidden by default, verified alongside the route's 403 so a guarded route with an ungated button is a defect; and any RBAC matrix refuses to lower the ceiling-defining admin role's own grant below its current level, client + save-API, so an operator can't drop the ceiling and self-lock the system — with the idempotent boot seed as the documented recovery; prior **v6.9**: the status surface splits in two: the internal ops-board stays the team's full-truth cockpit, and a separate **curated client-facing roadmap** — value buckets not phase IDs, machine tokens stripped, one honest positively-framed %, SOW-date milestones, only client-owed blockers as a courteous callout — becomes a PM deliverable the CS role forwards to the client; same verify-at-source facts, two readers, two stable URLs — `playbooks/status-surfaces-ops-and-client.md` owns the phase→value-bucket recipe + curation checklist; prior **v6.8**: interactive-UI floors: any drag-reorder uses a real DnD primitive with a keyboard sensor and the gate asserts sort_order persists across reload in BOTH directions — the hand-rolled down-direction no-op is auto-blocked (U5); object-page breadcrumb non-last crumbs are navigable links, not dead text (U6); a ported export screen's baked-in fixed max-width is reconciled against the shell content column as a legitimate deliberate deviation — the fidelity gate grew interaction teeth alongside its visual ones after a screen matched the export pixel-for-pixel yet was operationally broken; prior **v6.7**: front-load the cross-cutting floors: per-phase verifier gains Leg-5 Universal-UI (shared fidelity fixture, inherited not re-copied), Leg-6 Security (default-deny authz + fail-closed secrets), Leg-7 Index-discipline — the machine proves the whole class in-phase, the human confirms; Status Artifact: a live, always-current human tracking dashboard refreshed at each milestone — cockpit to the reports' flight recorder; Regression Ledger: every noted-and-fixed UI defect becomes a permanent machine assertion so it never returns as the run continues; UI phases carry the SRS-mandated cross-cutting NFRs
