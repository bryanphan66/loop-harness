...
Goal: hand the next `docs/WORKFLOW.md` step to the `stage-runner` subagent, then
summarise the result to the user. Keep this turn focused on orchestration — do
NOT do the step work in the main session.

Step IDs are `MACRO.STEP`: Pre-Build `1.1`–`1.15`, Build & Go-live `2.1`–`2.13`,
Post-Build `3.1`–`3.6` (`docs/WORKFLOW.md`).
...
2. Determine the next step to run:
   ...
   - If Current is marked done in Snapshot but the pointer still names it →
     advance per the `docs/WORKFLOW.md` order:
...
3. Read the matching goal text for the next step:
   - Prefer the `## Step <id>` block in `docs/STAGE_GOALS.md` if it exists.
   - Otherwise extract the step's row intent from `docs/WORKFLOW.md`
     (Role · Engine · Inputs · Output path · Gate) and pass that as the goal.
...
5. Spawn the stage-runner via the Task tool:
   ...
   Prompt template:
   ```
   Run step <id> (<step name>) end-to-end per docs/WORKFLOW.md.

   Goal (verbatim, substitutions applied):
   <substituted goal body, or the WORKFLOW row intent>
   ...
   Read STAGE.md, AGENTS.md, docs/WORKFLOW.md (your step row), docs/ROLE_MAP.md,
   docs/TRACE_SPEC.md, and your own agent definition first. Delegate to the
...
6. When stage-runner returns, do NOT repeat its full output. Quote ONLY the
   ...
   - `Status: MANUAL_CHECKPOINT_PENDING` → "Waiting on offline work — see
     MANUAL_CHECKPOINT above. The client-paging gate clears when the human
     returns the stated condition."
...
any 2.x build step before PB-G4 contract+deposit), STOP and run `/gate-check`
first — never start build code before PB-G4. Surface the missing gate to the
...
- Next step is a stubbed Build/Post-Build step with no built detail → say
  "step <id> detail is built in the next macro-stage increment; run inline or
  wait for that increment per `docs/WORKFLOW.md`."
