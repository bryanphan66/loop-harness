...
- [ ] **Build manifest complete** — `docs/build-manifest.md` exists per `docs/mau-tai-lieu/build-manifest.md`: **P0 (walking skeleton) defined**, every phase ≤ one agent session (≤~10 files), and the coverage checklist proves **every in-scope REQ-ID appears in exactly one phase**.
