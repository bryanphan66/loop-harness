...
5. `docs/about/ROLE_MAP.md` — which SDLC role plays each step and which agent + skill
   engine performs it.
6. `docs/about/TRACE_SPEC.md` — the token grammar (`GAP-NNN → REQ-ID → SC-NNN →
...
2. Locate the affected artifacts — BA spine (`docs/requirements/`), scope
   ...
   Design System Rule.
3. Check the token chain (`docs/about/TRACE_SPEC.md`): every feature traces
...
7. Record a **session trace** per `docs/about/TRACE_SPEC.md` before reporting done.
...
- The BA spine, scope baseline, RTM, stories, and verification register remain
  current.
- The token chain is unbroken (`docs/about/TRACE_SPEC.md`).
...
- A **session trace** is recorded per `docs/about/TRACE_SPEC.md`.
