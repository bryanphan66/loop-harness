...
|---|---|---|---|---|---|---|
| **2.0** | Kiểm tier-2 token chạy được với thư viện UI đã chọn | — | `tier2-ui-compat` ⚙️ | — | `check-tier2-ui-compat.mjs` | phiên bản build tool khớp major, mọi token thư viện đọc đều có trong tier 2, 0 cú pháp đời cũ |
...
- Lint chặn màu cứng (vd `no-raw-color` của reno-ui) **không** chặn được việc tự viết
  component mới. Gate lint theo vùng dựng ở 2.4 khi đã có app + eslint config — xem
  `macro-2-deltas.md` MD-02.
