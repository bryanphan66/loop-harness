...
|---|---|---|---|---|---|---|---|
| 2.0 | **Kiểm sẵn sàng trước khi xây** | Tech Lead | `check-tier2-ui-compat.mjs` + `check-dangling-refs.mjs` + `measure-macro2.mjs` | tier-2 tokens + `registry.json` của thư viện UI + Component Coverage Matrix | `docs/design/component-mapping-<thư-viện>.md` + `docs/macro2-run-log.md` + `docs/macro2-friction-log.md` | `tier2-ui-compat` ⚙️ + `dangling-refs` ⚙️ + component-mapping *(người)* **@một-lần** | no |
| 2.1 | Solution/data arch — **freeze ERD** | **SA** | `databases` + `tech-graph` | SRS(-lite) data-model + use-cases + screen inventory | `docs/decisions/<slug>.md` + `docs/system-architecture.md` (ERD) | ERD review: entities, normalization, audit + tenant fields; **ERD FROZEN** | no |
...
| 2.2 | Technical design + choose stack (TDR) | **Tech Lead** | `databases` + `ck-predict` | NFR + ERD | `docs/decisions/<slug>.md` (stack) + API contract | stack justified vs NFR (**default = the shipped walking-skeleton stack template**; deviations need the ADR to say why), API contract complete, authz; **STRIDE threat-model here** | no |
...
| 2.4 | **Walking skeleton** (manifest **P0**) + env + CI/CD + observability | DevSecOps | stack template `scaffold.sh` + `devops` + `deploy` | stack decision + manifest P0 | scaffolded monorepo + pipeline + compose + `components/ui/` kéo từ registry (`pnpm ui:sync`) | **WALKING SKELETON**: `install && build` green, `docker compose up` boots, health OK, CI(-equivalent local) green, secret scan clean; alerting/SLO live or N/A-by-decision | no |
| 2.5 | Seed + foundation data **(FOLDED→2.4)** | DevSecOps + Dev | `devops` *(seed đi cùng 2.4)* + seed-data-pattern | ERD + RBAC | seed scripts (extends the template's admin seed to the domain) | app boots with RBAC + **seeded admin login works**; FK-valid; P0 marked done in manifest | no |
...
| 2.10 | QA real-browser + video | QC/QA | `ck-code-review` + `web-testing` + `agent-browser` | running build + prototype exports | QA evidence + filled `docs/gates/visual-fidelity.md` | **DoD GATE** (`docs/gates/dod-build.md`): review + E2E + security + QA evidence + user-manual + design-system-compliance green per screen + **visual-fidelity pass per key screen** (fidelity assertions green + human side-by-side glance recorded); PUB product-shots captured after the APP screens they depict | no |
| 2.11 | Go-live readiness **(FOLDED→2.13)** | DevSecOps + PM | `ship` *(readiness đi cùng 2.13)* | build + infra | readiness checklist | readiness green; **rollback rehearsed**; **DR restore-drill + RTO/RPO (CONDITIONAL — N/A by decision)**; **NFR/load test (CONDITIONAL — N/A by decision)** | no |
| 2.12 | **UAT + sign-off (one client session)** | BA + Release Mgr + Client | — *(người phán: khách hoặc chủ ký)* → `delivery-closure-story/` | running build + prototype | `docs/uat/*` + signed sign-off (`locale-vi/`) | **ACCEPTANCE (CLIENT)**: critical journeys pass + matches prototype + sign-off signed (Lite: owner runs UAT + ack) | **yes** |
