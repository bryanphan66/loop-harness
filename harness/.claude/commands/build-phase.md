...
Goal: drive **one phase** of `docs/build/build-manifest.md` to done. This is the
...
1. **Preconditions** (read, don't assume):
   ...
   - `docs/build/build-manifest.md` must exist with a phase table/blocks. If
...
3. **Assemble the MINIMAL context packet** for the subagent — ONLY:
   - The phase's own block from `docs/build/build-manifest.md` (verbatim).
...
4. **Spawn ONE stage-runner** (subagent_type: `stage-runner`; alternatively
   ...
   stage-runner so gate + commit discipline is enforced):
   ...
   checkbox in docs/build/build-manifest.md, adds the 2.6/<P-id> History row
