...
|---|---|---|---|
| F1 | STAGE.md History "commit SHA" | A stage-boundary commit cannot know its own SHA; template shows `<sha>` per row with no convention. I used "— (this commit)" then back-filled the previous row's SHA in the next boundary commit. | Document the back-fill convention in docs/mau-tai-lieu/STAGE.md (or say "leave —; `git log --grep 'step X'` is the source"). |
