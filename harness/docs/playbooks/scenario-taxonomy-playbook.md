...
`SC-002`, …) per `docs/TRACE_SPEC.md`. The `REQ-traceback` column cites the
...
- **To RTM (1.7/2.12):** each SC-NNN lands in RTM Section 2 forward column against
  ...
  **incomplete RTM** (per `docs/TRACE_SPEC.md`).
...
- `docs/TRACE_SPEC.md` § Token Types — SC-NNN format + chain position.
- `docs/WORKFLOW.md` § 1.8 — the step this playbook owns.
...
- `docs/ROLE_MAP.md` — BA role + `ck-scenario` engine binding.
