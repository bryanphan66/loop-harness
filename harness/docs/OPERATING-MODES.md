...
**Harness TODO (packaging):** generalize this Mode-B tooling into `harness/templates/` + a `harness/docs/playbooks/steady-state-issue-pipeline.md` so a freshly-installed project gets Mode B out of the box, not just Mode A. Until then, Mode B is copied from elearning per project.
