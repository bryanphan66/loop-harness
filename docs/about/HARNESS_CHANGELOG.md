...
- **elearning adoption**: 13 `Module:` labels applied to 107 issues; org Module field **deleted** (needs `admin:org`); redundant `bug`/`enhancement`/`feature` labels removed (Type lives in the Issue Type field), `github`+`plane` kept; the module label is applied from the register's 23 sections mapped onto the 13 canonical module names (was written as `**Module:**` in the body). [correction 2026-09-01: an earlier draft credited a `feature-issues-sync.mjs` register->issue sync script for this — that script was never built; issues are authored via `new-issue.mjs`. See `feature-issue-ac-demo-standard.md`.]
